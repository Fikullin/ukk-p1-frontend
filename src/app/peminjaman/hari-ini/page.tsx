                ...
                {peminjaman.length === 0 ? (
                  ...
                  paginatedPeminjaman.map((item) => (
                    <Fragment key={item.id}>
                          ...
                          <>
                              ...
                              {item.return_status === 'pending' ? (
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className="text-green-600 hover:text-green-800 font-medium text-xs cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              <div className="flex space-x-2">
                                <button onClick={() => handleView(item)} className="text-blue-600 hover:text-blue-800" title="View">
                                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                    <path fillRule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd" />
                                  </svg>
                                </button>
                                {!item.deadline && !item.validated_by && (
                                  <button
                          ...
                          <>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              <div className="flex space-x-2">
                                ...
                                <button onClick={() => item.status === 'dipinjam' && item.return_status !== 'validated' ? handleStudentReturnRequest(item) : setToast({ message: 'Already returned or validated', type: 'info' })} className={`${item.status === 'dipinjam' && item.return_status !== 'validated' ? 'text-green-600 hover:text-green-800' : 'text-gray-400 cursor-not-allowed'}`} disabled={item.status !== 'dipinjam' || item.return_status === 'validated'} title="Return">
                                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                      ...
                      {expandedRows.includes(item.id) && (
                        <tr>
                          <td colSpan={userRole === 'siswa' ? 8 : 9} className="px-6 py-4 bg-gray-50">
